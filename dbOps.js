...
const URI_TO_CONNECT_MONGODB = "mongodb://localhost:27017/mydb";
...
let makeUpdateRoom = async (db, req, res, client, output) => {
  ...
  try {
    // initialize the bulk
    let bulk = await db.collection(COLLECTION_ROOMS).initializeOrderedBulkOp();
...
let makeUpdateRoomReadStatus = async (db, req, res, client, output) => {
  ...
  try {
    ...
    console.log(
      "read status is changed here, number of modified docs",
      docs.result.nModified
    );
    sendOutputAndCloseConnection(client, output, res);
  } catch (error) {
